...
function makeTicket(parts, index) { const ordered = parts.slice().sort((a, b) => { const rank = id => id === RELATION_POOL_ID ? -1 : POOL_IDS.indexOf(id); return rank(a.poolId) - rank(b.poolId); }); return Object.freeze({ version: VERSION, id: `${VERSION}:${ordered.map(item => `${item.poolId}:${item.tagId}`).join('|')}`, index, pools: Object.freeze(ordered.map(item => item.poolId)), tags: Object.freeze(ordered.map(item => item.tagId)), selections: Object.freeze(ordered.map(item => Object.freeze({ poolId: item.poolId, poolLabel: POOLS[item.poolId].label, tagId: item.tagId, label: item.label, prompt: item.prompt, tone: item.tone, intensity: item.intensity }))), snapshot: Object.freeze(ordered.map(item => `${POOLS[item.poolId].label}：${item.label}`).join('；')) }); }
