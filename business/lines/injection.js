...
export function createLinesInjectionController({ context, settings, enabled, readRaw, adultMode = () => 'off', direction = () => 'natural', promptTypes = {}, promptRoles = {}, clean = value => value } = {}) {
    const refresh = () => {
        ...
        const text = buildLinesInjection(lines.map(line => ({ ...line, next: line.next ? prefixNext(line.next, line.stall) : line.next })), { adultMode: adultMode(), direction: direction() }).replace(/\r/g, '').replace(/  (.*)/g, (_, value) => `  ${clean(value)}`);
