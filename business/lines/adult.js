...
function baseAdultPromptGuidance(mode) {
    ...
    const label = normalized === 'dominant' ? '成人主导模式' : '混合模式';
    const target = normalized === 'dominant' ? '70%' : '30%';
    return `
【成人追加合同·${label}】本地分池目标约 ${target} 成人线；这是选材目标，不是硬配额，证据不足允许低于目标，不为比例造人、串味或凑数。仅适用于明确成年且自愿的虚构角色。SFW 票不得成人化；使用 NSFW 票的新线必须由成人欲望、成人场景或成人互动本身驱动，并落实票内的驱动力、行为、节奏、场景与后果。已有非成人线不得强行成人化。成人线的 Next 必须写具体玩法/行为、实际主动方、其他参与者的明确主动回应，以及所选动作的即时身体感受、选择变化或关系压力；不得用暧昧、含糊或淡出回避已建立的成人推进。高潮、虚脱、留宿、遣回、依赖加深或关系升温只能作为后果，不能单独构成 Next。成人线可为 1v1、1vN 或 NvN，但不得凭空改变人物边界。`;
}
