...
function parseLegacyInner(content) {
    ...
    for (const source of String(content).split(/\r?\n/)) {
        ...
        else if (current && /^Cue\s*:/i.test(text)) current.cue = text.replace(/^Cue\s*:\s*/i, '').trim();
    }
...
export function validateLinesResponse(raw) {
    ...
    const scanned = strictBlocks(match[1]); if (!scanned.ok || !scanned.blocks.length) return { ok: false, reason: scanned.reason || 'no-lines' };
    const model = [];
