...
export function createAxisTransactionController(env = {}) {
    const commit = async cal => {
        const chatId = env.chatId?.(), items = env.items?.() || [], conflicts = env.conflicts?.(items, cal) || [], key = env.charKey?.(), raw = key ? env.anchor?.(key) : null;
        ...
        if (conflicts.length || anchorConflict) {
            ...
            if (!action || action === 'cancel' || env.chatId?.() !== chatId) return { ok: false, cancelled: true };
        }
        const ids = new Set(conflicts.map(c => c.item.id)), fixed = new Map(conflicts.map(c => [c.item.id, c.fixed]));
        const nextItems = action === 'delete' ? items.filter(item => !ids.has(item.id)) : items.map(item => fixed.get(item.id) || item);
        if (env.chatId?.() !== chatId) return { ok: false, cancelled: true };
        ...
        if (anchorConflict && key) { const fixedMonth = Math.min(Math.max(Number(raw.month) || 1, 1), env.monthCount(cal)); const fixedDay = Math.min(Math.max(Number(raw.day) || 1, 1), env.monthDays(cal, fixedMonth)); const result = action === 'delete' ? env.setAnchor?.(key, null) : env.setAnchor?.(key, fixedMonth, fixedDay); if (!result?.ok) return { ok: false, error: '当前聊天无法写入日期锚点' }; }
