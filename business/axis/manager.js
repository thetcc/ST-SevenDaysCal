...
export function createCalendarManager(env = {}) {
    ...
    const captureDraft = () => {
        ...
        if (fields) {
            ...
            m.draft = {
                kind: previous.kind, id: previous.id, revision: previous.revision, weekdayCycle: previous.weekdayCycle, displayStyle: fields.displayStyle === 'classical' ? 'classical' : 'numeric',
    ...
    const renderCard = () => {
        ...
        return `<section class="sp-alm-manager-card"><div class="sp-alm-manager-card-head"><div class="sp-alm-manager-card-title">编辑当前历法</div>${actions}</div><div class="sp-alm-manager-edit-fields">${m.error ? `<div class="sp-alm-manager-error" role="alert"><i class="fa-solid fa-triangle-exclamation" aria-hidden="true"></i> ${env.escapeHtml(m.error)}</div>` : ''}<label class="sp-alm-field"><span>纪年名 <small>选填</small></span><input id="sp-alm-manager-era" maxlength="${env.limits.eraNameLength}" value="${env.escapeAttr(cal.era)}"></label><label class="sp-alm-field"><span>日期显示</span><select id="sp-alm-manager-display-style" aria-label="日期显示风格"><option value="numeric"${cal.displayStyle !== 'classical' ? ' selected' : ''}>数字</option><option value="classical"${cal.displayStyle === 'classical' ? ' selected' : ''}>古典中文</option></select></label>${rows}<button class="sp-alm-manager-add-month" type="button"><i class="fa-solid fa-plus" aria-hidden="true"></i><span>添加月份</span></button></div></section>`;
    };
