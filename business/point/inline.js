...
export function createPointInlineRenderer(env) {
    ...
    function buildScheduleDay(dayKey, rawArg = null, readOnly = false, calendarOverride = null, weekdayRefOverride = undefined) {
        ...
        const rows = events.map((event, eventIndex) => {
            ...
            const actions = readOnly ? '' : `<span class="sp-sch-drawer-actions">${env.makeInjectBtn(env.buildPointInjectText(event, weather, temp, dateLabel, env.pointOwner?.()))}<button class="sp-sch-del-one" data-day="${env.escapeAttr(String(dayKey))}" data-ev="${eventIndex}" title="删除这个点"><i class="fa-solid fa-xmark"></i></button></span>`;
