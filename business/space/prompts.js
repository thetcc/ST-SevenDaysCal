...
· 设置入口：主面板里的齿轮「设置」。设置从上到下分五组：总开关 / 通用设置 / 注入与内容设置 / 推进设置 / 数据设置。
...
· 线（设置→注入与内容设置 / 推进设置）：「启用平行事件（线）」「潜伏注入主楼 AI」「虚线·冷知识」「推进策略（回合/时间/手动）+ 间隔」。线以 UC（用户核心角色）为主轴，也会额外放行 1-2 条**非 UC 的配角/NPC 支线**（重要配角自己的小线索，须同一世界观、同一叙事尺度，不会跨尺度乱入）。
· 面（设置→注入与内容设置 / 推进设置）：「大纲自动注入」+ 判定间隔。
...
· 楼内渲染框（设置→通用设置→显示与通知）：主开关「楼内渲染框」，下面有子开关分别控制 点/线/轴/标注池/召回 这几个框显不显；主开关关了子开关全失效。
· 界面字号：设置→通用设置→显示与通知里的 −／＋ 步进，独立于酒馆自身的字号。
· 通知档位：关／简约／全量三档。`;
